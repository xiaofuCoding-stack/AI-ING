---
name: tech-analyzer
description: 分析project目录下的开源技术项目，提取核心技术原理、架构模式和关键实现细节。自动生成技术文档到sumup目录；每当有新技术/新项目进入时，同步更新「每日前沿」文档，保持每日更新。当用户需要理解project目录下技术的核心原理、提取技术要点或生成技术总结文档时使用。
---
...
本技能用于分析`project/`目录下的开源技术项目，提取其核心技术原理、架构模式和实现细节，然后自动生成技术文档到`sumup/`目录。**每当分析新项目、或发现/引入新技术时，必须同步更新 `sumup/每日前沿.md`，按日记录「当天最炸裂的发布」等，保证每日更新。**
...
### 4. 更新「每日前沿」文档（必须，每日更新）

**每当分析新项目、或引入/发现新技术时，必须同步更新 `sumup/每日前沿.md`。**

- **时机**：在生成/更新 `sumup/<项目名>/` 下技术文档的同一轮操作中，一并更新每日前沿。
- **位置**：在 `sumup/每日前沿.md` 的「按日汇总」下，找到或新增「今日」的日期区块（如 `#### YYYY-MM-DD（周X）`），在其表格中**追加一行**。
- **格式**：`| 类型 | 名称 | 一句话 | 链接 |`  
  - 类型：填「项目」/「模型」/「框架」/「动态」之一。  
  - 名称：项目或技术名称。  
  - 一句话：用一句话说明「为什么算当天最炸裂 / 值得跟」。  
  - 链接：本仓库内文档相对路径（如 `[分析与部署指南](clawdbot-分析与部署指南.md)`）或外部链接；无则留空或写「-」。
- **规则**：  
  - 同一自然日有多条时，都写在当日同一日期区块的同一张表里。  
  - 新日期区块插在「按日汇总」内、**最新日期在最上**（即新日期写在现有示例/历史之上）。  
  - 保证至少「每日一更」：有新技术进来就立即追加；若当天尚无任何条目，也要为今天新建一个日期区块并写一条。
...
**注意**：脚本本身不会写 `sumup/每日前沿.md`。使用本技能的人或 AI 在完成分析后，**必须**按「4. 更新每日前沿文档」的规则，手工或在同一轮对话中编辑 `sumup/每日前沿.md`，把本条技术追加到当日区块，保证每日更新。
